...
struct SelectorScrollView : public FixedSizeView {
private :
	UI::FlexibleString<SelectorScrollView> title;
	
	static constexpr double MARGIN_BUTTON_RATIO = 5.0;
public :
	using CallBackFuncType = std::function<void (const SelectorScrollView &value)>;
	...
	int button_num;
	std::vector<UI::FlexibleString<SelectorScrollView> > button_texts;
	...
	SelectorScrollView (double x0, double y0, double width) : View(x0, y0), FixedSizeView(x0, y0, width) {}
	virtual ~SelectorScrollView () {}
	...
	SelectorScrollView *set_texts(const std::vector<UI::FlexibleString<SelectorScrollView> > &button_texts, int init_selected) { // mandatory
	...
	/*
	SelectorScrollView *set_texts(const std::vector<std::function<std::string (const SelectorScrollView &)> > &button_texts, int init_selected) { // mandatory
		this->button_num = button_texts.size();
		this->button_texts.resize(button_num);
		for (int i = 0; i < button_num; i++) this->button_texts[i] = UI::FlexibleString<SelectorScrollView>(button_texts[i], *this);
		this->selected_button = init_selected;
		return this;
	}*/
	SelectorScrollView *set_title(const std::string &title) {
		this->title = UI::FlexibleString<SelectorScrollView>(title);
		return this;
	}
	SelectorScrollView *set_title(std::function<std::string (const SelectorScrollView &)> title_func) {
		this->title = UI::FlexibleString<SelectorScrollView>(title_func, *this);
		return this;
	}
	SelectorScrollView *set_on_change(CallBackFuncType on_change_func) {
